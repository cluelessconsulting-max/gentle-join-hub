...
function buildHtml(isWaitlist: boolean, eventName: string, eventDate: string, eventLocation: string) {
  const badge = isWaitlist ? 'WAITLIST CONFIRMATION' : 'APPLICATION RECEIVED';
  const headline = isWaitlist ? "You're on the waitlist." : 'Your application has been submitted.';
  ...
  const locationLine = eventLocation ? ' &middot; ' + eventLocation : '';
    ...
    <tr><td align="center">
      <table width="520" cellpadding="0" cellspacing="0" style="background-color:#EDE8E0;">
        ...
        <tr><td style="padding:0 0 10px 0;text-align:center;">
          <span style="font-size:9px;letter-spacing:3px;text-transform:uppercase;color:#B49A6A;">${badge}</span>
        </td></tr>
        <tr><td style="padding:0 0 24px 0;text-align:center;">
          <span style="font-size:28px;font-weight:300;color:#0A0A0A;letter-spacing:0.5px;">${headline}</span>
        </td></tr>
        ...
        <tr><td style="padding:0 0 32px 0;text-align:center;">
          <span style="font-size:11px;color:#8B8178;letter-spacing:0.5px;">${eventDate || ''}${locationLine}</span>
        </td></tr>
        <tr><td style="padding:0 0 24px 0;"><div style="height:1px;background-color:rgba(10,10,10,0.12);"></div></td></tr>
        <tr><td style="padding:0 0 24px 0;text-align:center;">
          <span style="font-size:13px;color:#8B8178;letter-spacing:0.3px;line-height:1.8;">${bodyText}</span>
        </td></tr>
        ${noTickets}
        <tr><td style="padding:0 0 12px 0;"><div style="height:1px;background-color:rgba(10,10,10,0.12);"></div></td></tr>
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const htmlContent = buildHtml(isWaitlist, eventName, eventDate || '', eventLocation || '');
    ...
    await fetch('https://api.brevo.com/v3/contacts', {
      ...
      body: JSON.stringify({
        email,
        attributes: { LAST_EVENT: eventName },
        listIds: [],
        updateEnabled: true,
      }),
    });
